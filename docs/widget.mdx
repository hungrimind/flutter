...
Whenever you create a flutter project the started applications is a counter app with a very simple `StatefulWidget` that increments the counter. Whenever you press the `+` icon, in increments the `_counter` value and calls `setState` to trigger a rebuild of the `StatefulWidget` and you see the counter value change.
...
<img src="/flutter/buildcontext/element_tree.png" alt="element tree relating to widget tree"/>
